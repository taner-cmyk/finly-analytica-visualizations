...
let debugText = "Yükleniyor...";

// 1. Kök elementi (Root) güvenli şekilde oluşturma
function getRoot() {
    let root = document.getElementById("finly-ticker-root");
    if (!root) {
        root = document.createElement("div");
        root.id = "finly-ticker-root";
        // Boyutları %100 kapsayacak şekilde ayarlıyoruz
...
// 2. Kayan yazıyı (Ticker) ekrana çizdirme
function renderTicker() {
    const root = getRoot();

    try {
        // Veri objesinin ve tablonun varlığını kontrol et
        if (!currentData || !currentData.tables || !currentData.tables.DEFAULT) {
            debugText = "Veri kaynağı bekleniyor...";
            root.innerHTML = `<div style="color:red; font:12px Arial; padding:10px;">${debugText}</div>`;
            return;
        }

        const table = currentData.tables.DEFAULT;

        // Sağ panelden Boyut ve Metrik eklenip eklenmediğini kontrol et
        if (table.length === 0) {
            debugText = "Veri boş. Sağ panelden Hisse (Boyut), Fiyat (Metrik) ve Değişim (Metrik) ekleyin.";
            root.innerHTML = `<div style="color:orange; font:12px Arial; padding:10px;">${debugText}</div>`;
            return;
        }

        // Gelen verileri döngüye alıp yan yana metin olarak birleştir
        const textElements = table.map((row) => {
            // ticker-config.json içindeki id'lere göre çekiyoruz
            const code = row.tickerStock ? String(row.tickerStock[0] ?? "") : "";
            const valRaw = row.tickerLast ? row.tickerLast[0] : null;
            const changeRaw = row.tickerChange ? row.tickerChange[0] : null; // Değişim verisi

            const valNum = Number(valRaw);
            const val = Number.isFinite(valNum) ? valNum.toFixed(2) : String(valRaw ?? "");

            // Renk, Ok ve Yüzde mantığı
            let changeHtml = "";
            if (changeRaw !== null && changeRaw !== "") {
                const changeNum = Number(changeRaw);
                if (Number.isFinite(changeNum)) {

                    // Looker Studio ondalık gönderdiği için 100 ile çarpıyoruz (ör: 0.10 -> 10)
                    const yuzdeDeger = changeNum * 100;

                    if (yuzdeDeger > 0) {
                        // Artış: Yeşil ve Yukarı Ok
                        changeHtml = `<span style="color: #0f9d58; font-weight: bold;">(▲ %${yuzdeDeger.toFixed(2)})</span>`;
                    } else if (yuzdeDeger < 0) {
                        // Azalış: Kırmızı ve Aşağı Ok (Eksiyi kaldırıp okla gösteriyoruz)
                        changeHtml = `<span style="color: #d23f31; font-weight: bold;">(▼ %${Math.abs(yuzdeDeger).toFixed(2)})</span>`;
                    } else {
                        // Nötr: Gri
                        changeHtml = `<span style="color: #5f6368; font-weight: bold;">(- %0.00)</span>`;
                    }
                }
            }

            return code ? `<span style="font-weight:bold;">${code}:</span> ${val} ${changeHtml}` : "";
        }).filter(Boolean).join("&nbsp;&nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;&nbsp;&nbsp;");

        debugText = "Veri başarıyla yüklendi.";

        // 1. Animasyonlu HTML yapısını ekrana bas
        root.innerHTML = `
          <div class="ticker">
            <div class="line" id="ticker-line">
              ${textElements}
            </div>
          </div>
        `;

        // 2. DİNAMİK HIZ AYARI (Hisse sayısına göre otomatik hesaplar)
        // Her hissenin ekranda okunma süresi (ortalama 4 saniye)
        const hisseBasinaSaniye = 4;
        const toplamSure = table.length * hisseBasinaSaniye;

        // CSS animasyon süresini JS ile anlık olarak güncelliyoruz
        document.getElementById("ticker-line").style.animationDuration = toplamSure + "s";

    } catch (e) {
        debugText = "Hata: " + (e && e.message ? e.message : String(e));
        root.innerHTML = `<div style="color:red; font:12px Arial; padding:10px;">${debugText}</div>`;
    }
}

// 3. Looker Studio'dan veri geldiğinde tetiklenen ana fonksiyon
function drawViz(vizData) {
    currentData = vizData;
    renderTicker();
}

// 4. Looker Studio (DSCC) aboneliği
