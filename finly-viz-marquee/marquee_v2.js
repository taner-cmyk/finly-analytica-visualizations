...
function renderTicker() {
    ...
    try {
        if (!currentData || !currentData.tables || !currentData.tables.DEFAULT) {
            root.innerHTML = `<div style="color:gray; font:12px Arial; padding:10px;">Veri kaynağı bekleniyor...</div>`;
            return;
        }
        ...
        if (table.length === 0) {
            root.innerHTML = `<div style="color:orange; font:12px Arial; padding:10px;">Lütfen sağ panelden verileri ekleyin.</div>`;
            return;
        }

        // Verileri birleştirme
        const textElements = table.map((row) => {
            // IDs: tickerStock, tickerLast, tickerChange (Config dosyanla aynı olmalı)
            const code = row.tickerStock ? String(row.tickerStock[0] ?? "") : "";
            const valRaw = row.tickerLast ? row.tickerLast[0] : null;
            const changeRaw = row.tickerChange ? row.tickerChange[0] : null;

            const valNum = Number(valRaw);
            const val = Number.isFinite(valNum) ? valNum.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : String(valRaw ?? "");

            // Renk ve Yüzde Mantığı
            let changeHtml = "";
            if (changeRaw !== null && changeRaw !== "") {
                const changeNum = Number(changeRaw);
                if (Number.isFinite(changeNum)) {
                    // Veriyi yüzdeye çeviriyoruz (0.01 -> 1.00)
                    const yuzdeDeger = changeNum * 100;

                    if (yuzdeDeger > 0) {
                        // ARTIS: Yeşil (#0f9d58)
                        changeHtml = `<span style="color: #0f9d58; font-weight: bold; margin-left: 5px;">▲ %${yuzdeDeger.toFixed(2)}</span>`;
                    } else if (yuzdeDeger < 0) {
                        // AZALIS: Kırmızı (#d23f31)
                        changeHtml = `<span style="color: #d23f31; font-weight: bold; margin-left: 5px;">▼ %${Math.abs(yuzdeDeger).toFixed(2)}</span>`;
                    } else {
                        // NOTR: Gri
                        changeHtml = `<span style="color: #5f6368; font-weight: bold; margin-left: 5px;">- %0.00</span>`;
                    }
                }
            }

            return code ? `<span style="white-space:nowrap; font-family: sans-serif; font-size: 14px;">
                            <b style="color:#333;">${code}:</b> ${val} ${changeHtml}
                           </span>` : "";
        }).filter(Boolean).join("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");

        // HTML Yapısı
        root.innerHTML = `
          <div class="ticker" style="width:100%; overflow:hidden; white-space:nowrap;">
            <div class="line" id="ticker-line" style="display:inline-block; will-change:transform;">
        ...
        const toplamSure = Math.max(table.length * hisseBasinaSaniye, 10);
        const line = document.getElementById("ticker-line");
        line.style.animation = `finlyTickerScroll ${toplamSure}s linear infinite`;

        // CSS Animasyonunu Dinamik Olarak Ekle (Eğer yoksa)
        if (!document.getElementById("ticker-animation-style")) {
            const style = document.createElement("style");
            style.id = "ticker-animation-style";
            style.innerHTML = `
                @keyframes finlyTickerScroll {
        ...
        root.innerHTML = `<div style="color:red; font:12px Arial; padding:10px;">Hata: ${e.message}</div>`;
    }
}
