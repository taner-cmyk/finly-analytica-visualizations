...
function getRoot() {
    let root = document.getElementById("heatmap-root");
    if (!root) {
        root = document.createElement("div");
        root.id = "heatmap-root";
...
// Orijinal TradingView Renk Paleti
function getColor(change) {
    if (change === null || isNaN(change)) return "#434651"; // Gri (Nötr)

    // Uç değerler daha koyu, orta değerler daha parlak
    if (change >= 8) return "#006442";      // Çok Artan: Koyu TV Yeşili
    if (change > 0) return "#089981";       // Artan: Parlak TV Yeşili
    if (change <= -8) return "#99202c";     // Çok Düşen: Koyu TV Kırmızısı
    if (change < 0) return "#f23645";       // Düşen: Parlak TV Kırmızısı

    return "#434651"; // Tam 0 ise
}

// Kusursuz Kareleştirme (Balanced Partition Treemap) Algoritması
function partitionTreemap(nodes, x, y, w, h) {
    if (!nodes || nodes.length === 0) return;

    // Eğer tek bir node kaldıysa, tüm alanı ona ver
    if (nodes.length === 1) {
        let n = nodes[0];
        n.x0 = x; n.y0 = y; n.x1 = x + w; n.y1 = y + h;

        // Eğer bu bir Sektörse (altında hisseler varsa), içini de hesapla
        if (n.children && n.children.length > 0) {
            let pTop = (n.name !== "Root") ? 22 : 0; // TV için başlık yüksekliği 22px
            n.children.sort((a, b) => b.value - a.value); // Büyükler başa (Piyasa Değerine göre)
            partitionTreemap(n.children, x, y + pTop, w, h - pTop);
        }
        return;
    }

    // Node'ları ortadan ikiye, değerleri en eşit olacak şekilde böl
    ...
    for (let i = 0; i < nodes.length; i++) {
        let nextSum = leftSum + nodes[i].value;
        if (nextSum >= half) {
            if (i === 0) { splitIndex = 1; leftSum = nextSum; }
            else if (Math.abs(half - leftSum) < Math.abs(half - nextSum)) {
                splitIndex = i;
            } else {
                splitIndex = i + 1;
                leftSum = nextSum;
    ...
    // Güvenlik: Asla 0'a bölme
    if (splitIndex === 0 || splitIndex === nodes.length) {
        splitIndex = Math.max(1, Math.floor(nodes.length / 2));
        leftSum = nodes.slice(0, splitIndex).reduce((sum, n) => sum + n.value, 0);
    }

    let ratio = leftSum / total;
    let leftNodes = nodes.slice(0, splitIndex);
    let rightNodes = nodes.slice(splitIndex);

    // Alanı uzun olan kenardan böl (Bu sayede çubuklar değil, kareler oluşur)
    if (w > h) {
        let wLeft = w * ratio;
        partitionTreemap(leftNodes, x, y, wLeft, h);
        partitionTreemap(rightNodes, x + wLeft, y, w - wLeft, h);
    } else {
        let hTop = h * ratio;
        partitionTreemap(leftNodes, x, y, w, hTop);
        partitionTreemap(rightNodes, x, y + hTop, w, h - hTop);
    }
}

function getLeaves(node, leaves = []) {
    if (!node.children || node.children.length === 0) {
        if (node.name !== "Root") leaves.push(node);
    } else {
...
function renderHeatmap() {
    ...
    try {
        if (!currentData || !currentData.tables || !currentData.tables.DEFAULT || currentData.tables.DEFAULT.length === 0) {
            root.innerHTML = `<div style="color:orange; padding:20px; font-family:sans-serif;">Veri bekleniyor... Sağ panelden Piyasa Değeri ve Değişim metriklerini ekleyin.</div>`;
            return;
        }

        const table = currentData.tables.DEFAULT;
        const dimsCount = currentData.fields.dimensions ? currentData.fields.dimensions.length : 0;

        if (dimsCount === 0) return;

        let hierarchyData = { name: "Root", children: [], value: 0 };

        // Veriyi Hiyerarşik Yapıya Çevir
        table.forEach(row => {
            const dims = row.dimensions || [];
            const mets = row.metrics || [];

            // 1. Metrik: Büyüklük (Piyasa Değeri / Kapanış / Hacim)
            let size = Math.max(0.01, Number(mets[0] || 0));

            // 2. Metrik: Değişim % (Renk için)
            let changeRaw = Number(mets[1] || 0);
            let change = (changeRaw > -1 && changeRaw < 1 && changeRaw !== 0) ? changeRaw * 100 : changeRaw;

            let currentLevel = hierarchyData;
            currentLevel.value += size;

            // Sektör/Kırılım seviyelerini oluştur
            for (let i = 0; i < dimsCount - 1; i++) {
                let groupName = String(dims[i] || "Bilinmeyen Sektör");
                let existingGroup = currentLevel.children.find(c => c.name === groupName && c.children);

                if (!existingGroup) {
                    existingGroup = { name: groupName, children: [], value: 0 };
                    currentLevel.children.push(existingGroup);
                }
                existingGroup.value += size;
                currentLevel = existingGroup;
            }

            // En son yaprak (Hisse)
            let leafName = String(dims[dimsCount - 1] || "Bilinmeyen Hisse");
            currentLevel.children.push({ name: leafName, value: size, change: change });
        });

        root.innerHTML = "";

        // Veriyi büyükten küçüğe sırala (Piyasa Değerine Göre) ve Kareleştirme Algoritmasını başlat
        hierarchyData.children.sort((a, b) => b.value - a.value);
        partitionTreemap([hierarchyData], 0, 0, width, height);

        const leaves = getLeaves(hierarchyData);

        // Sektör Başlıkları (Varsa)
        if (dimsCount > 1) {
            hierarchyData.children.forEach(sectorNode => {
                // Başlıkların çizilebilmesi için kutunun minimum genişliğe ulaşmış olması gerekir
                if ((sectorNode.x1 - sectorNode.x0) > 40 && (sectorNode.y1 - sectorNode.y0) > 22) {
                    const header = document.createElement("div");
                    header.className = "sector-header";
                    header.style.left = sectorNode.x0 + "px";
                    header.style.top = sectorNode.y0 + "px";
                    header.style.width = (sectorNode.x1 - sectorNode.x0) + "px";
                    header.style.height = "22px"; // Sabit yükseklik
                    header.innerText = sectorNode.name + " >"; // TV tarzı ok işareti
                    root.appendChild(header);
                }
            });
        }

        // Hisseleri (Kareleri) Çiz
        leaves.forEach(leaf => {
            const box = document.createElement("div");
            box.className = "node";
            box.style.left = leaf.x0 + "px";
            box.style.top = leaf.y0 + "px";
            box.style.width = (leaf.x1 - leaf.x0) + "px";
            box.style.height = (leaf.y1 - leaf.y0) + "px";
            box.style.backgroundColor = getColor(leaf.change);

            const boxWidth = leaf.x1 - leaf.x0;
            const boxHeight = leaf.y1 - leaf.y0;

            const changeStr = leaf.change > 0 ? `+${leaf.change.toFixed(2)}%` : `${(leaf.change || 0).toFixed(2)}%`;

            // Kutu boyutuna göre dinamik yazı yerleştirme
            if (boxWidth > 50 && boxHeight > 40) {
                // Büyük kutular
                box.innerHTML = `
                    <div class="ticker-name">${leaf.name}</div>
                    <div class="ticker-change">${changeStr}</div>
                `;
            } else if (boxWidth > 35 && boxHeight > 25) {
                // Orta kutular
                box.innerHTML = `
                    <div class="ticker-name" style="font-size: 10px;">${leaf.name}</div>
                    <div class="ticker-change" style="font-size: 9px; margin-top: 0px;">${changeStr}</div>
                `;
            } else if (boxWidth > 20 && boxHeight > 15) {
                // Çok küçük kutular
                box.innerHTML = `
                    <div class="ticker-name" style="font-size: 8px;">${leaf.name}</div>
                `;
            }

            // Mouse ile üzerine gelince tooltip göster
            box.title = `${leaf.name} | Değişim: %${(leaf.change || 0).toFixed(2)}`;
            root.appendChild(box);
        });

    } catch (e) {
        root.innerHTML = `<div style="color:red; padding:20px;">Hata: ${e.message}</div>`;
    }
}
...
// dscc kütüphanesini dinleme başlat
