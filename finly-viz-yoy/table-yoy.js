...
function renderTable() {
    ...
    // 1. TARİH: Her zaman listedeki en büyük (max) tarih
    if (!userSelectedDate1 || !availableDates.includes(userSelectedDate1)) {
        userSelectedDate1 = availableDates[0];
    }

    // 2. TARİH (GELİR TABLOSU MANTIĞI): Geçen Yılın Aynı Dönemi
    if (!userSelectedDate2 || !availableDates.includes(userSelectedDate2)) {
        let d1 = String(userSelectedDate1);
        if (d1.length === 8) {
            // İlk seçili tarihin yılını bul, 1 çıkar, ay/gün kısmını aynen ekle
            let prevYearDate = String(Number(d1.substring(0, 4)) - 1) + d1.substring(4, 8);

            if (availableDates.includes(prevYearDate)) {
                userSelectedDate2 = prevYearDate; // Geçen yıl verisi varsa onu seç
            } else {
                userSelectedDate2 = availableDates[1] || availableDates[0]; // Yoksa bir öncekine düş
            }
        } else {
            userSelectedDate2 = availableDates[1] || availableDates[0]; // Format 8 hane değilse
        }
    }
    ...
    let html = `
        <table>
            <thead>
                <tr>
                    <th>Özet Gelir Tablosu<span class="sub-title">Bin TRY</span></th>
                    <th>
        ...
        const v1 = rowData1 ? Number(rowData1.metrics[i] || 0) : null;
        const v2 = rowData2 ? Number(rowData2.metrics[i] || 0) : null;
        const p = (v1 !== null && v2) ? Math.round(((v1 - v2) / Math.abs(v2)) * 100) : null;
        return `<tr>
                        <td>${m.name}</td>
                        <td>${v1 !== null ? v1.toLocaleString('tr-TR') : '-'}</td>
                        <td>${v2 !== null ? v2.toLocaleString('tr-TR') : '-'}</td>
    ...
    document.getElementById('trigger-1').addEventListener('click', (e) => {
        e.stopPropagation(); // Tıklamanın dışarıya yayılmasını engeller
    ...
    document.querySelectorAll('.date-opt-1').forEach(item => {
        item.addEventListener('click', () => {
            userSelectedDate1 = item.getAttribute('data-value');
            renderTable();
        });
    });
    ...
    document.querySelectorAll('.date-opt-2').forEach(item => {
        item.addEventListener('click', () => {
            userSelectedDate2 = item.getAttribute('data-value');
            renderTable();
        });
    });
}
...
// 2. İFRAME DIŞINA (Looker raporundaki başka bir yere) TIKLAYINCA KAPATMA
