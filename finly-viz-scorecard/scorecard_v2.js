...
let isInitialLoad = true; // YENİ: Raporun ilk açılışını takip eder

function getRoot() {
  let root = document.getElementById("finly-tabs-root");
  if (!root) {
    root = document.createElement("div");
    root.id = "finly-tabs-root";
...
function renderTabs() {
  const root = getRoot();
  try {
    if (!currentData || !currentData.tables || !currentData.tables.DEFAULT) {
      root.innerHTML = `<div style="color:gray; font:12px Arial;">Veri bekleniyor...</div>`;
      return;
    }
    ...
    if (table.length === 0 || !fields.periodDims || !fields.changeMets) {
      root.innerHTML = `<div style="color:orange; font:12px Arial;">Lütfen sağ panelden Filtreleri ve Metrikleri ekleyin.</div>`;
      return;
    }

    const dimFields = fields.periodDims;
    const metFields = fields.changeMets;

    // Verileri topluyoruz
    let totals = metFields.map(() => 0);
    table.forEach(row => {
      const rowMets = row.changeMets || [];
      metFields.forEach((_, i) => {
        totals[i] += Number(rowMets[i] || 0);
      });
    });

    // ==============================================================
    // YENİ: İLK AÇILIŞTA "1H" KUTUSUNU OTOMATİK SEÇME MANTIĞI
    // ==============================================================
    if (isInitialLoad && metFields.length > 0) {
      isInitialLoad = false; // Sadece 1 kez çalışması için kapatıyoruz (Sonsuz döngüyü önler)

      // İsmi "1H" veya "1 H" olan metriğin sırasını (index) bul
      let defaultIndex = metFields.findIndex(f => {
        let upperName = f.name.toUpperCase();
        return upperName.includes("1H") || upperName.includes("1 H");
      });

      // Eğer kazara 1H eklenmemişse, varsayılan olarak en baştaki (0.) kutuyu seç
      if (defaultIndex === -1) defaultIndex = 0;

      activeIndex = defaultIndex;
      const conceptId = dimFields[defaultIndex] ? dimFields[defaultIndex].id : null;

      if (conceptId) {
        let exactTrue = "true";
        for (let r = 0; r < table.length; r++) {
          let val = table[r].periodDims ? String(table[r].periodDims[defaultIndex]) : "";
        ...
        // Looker Studio'nun grafikleri tam yüklemesini bekleyip (500ms) filtreyi tetikle
        setTimeout(() => {
          dscc.sendInteraction("periodFilter", dscc.InteractionType.FILTER, {
            concepts: [conceptId],
            values: [[exactTrue]]
          });
        }, 500);
    ...
    // Kutuları oluşturma döngüsü
    metFields.forEach((metField, index) => {

      let rawLabel = metField.name.toUpperCase();
      let periodLabel = metField.name;

      if (rawLabel.includes("1A") || rawLabel.includes("1 A")) periodLabel = "1 Ay";
      else if (rawLabel.includes("3A") || rawLabel.includes("3 A")) periodLabel = "3 Ay";
      else if (rawLabel.includes("6A") || rawLabel.includes("6 A")) periodLabel = "6 Ay";
      else if (rawLabel.includes("1H") || rawLabel.includes("1 H")) periodLabel = "1 Hafta";
      else if (rawLabel.includes("1Y") || rawLabel.includes("1 Y")) periodLabel = "1 Yıl";
      else if (rawLabel.includes("3Y") || rawLabel.includes("3 Y")) periodLabel = "3 Yıl";
      else if (rawLabel.includes("5Y") || rawLabel.includes("5 Y")) periodLabel = "5 Yıl";
      else if (rawLabel.includes("YTD") || rawLabel.includes("YBB")) periodLabel = "YTD";
      else if (rawLabel.includes("GÜN") || rawLabel.includes("GUN")) periodLabel = "Gün İçi";

      const changeNum = totals[index] * 100;
      let changeClass = "val-neutral";
      let changeFormatted = "% 0,00";

      if (Number.isFinite(changeNum)) {
        const formattedNum = changeNum.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        if (changeNum > 0) {
          changeClass = "val-up";
          changeFormatted = "% " + formattedNum;
        } else if (changeNum < 0) {
          changeClass = "val-down";
          changeFormatted = "% " + formattedNum;
        }
      }

      const box = document.createElement("div");
      box.className = "finly-box" + (activeIndex === index ? " active" : "");
      box.innerHTML = `
                <div class="period-label">${periodLabel}</div>
                <div class="change-val ${changeClass}">${changeFormatted}</div>
            `;

      const conceptId = dimFields[index] ? dimFields[index].id : null;
      ...
      box.addEventListener("click", () => {
        ...
        if (activeIndex === index) {
          activeIndex = null;
          dscc.sendInteraction("periodFilter", dscc.InteractionType.FILTER, undefined);
        } else {
          activeIndex = index;
          let exactTrue = "true";
          for (let r = 0; r < table.length; r++) {
            let val = table[r].periodDims ? String(table[r].periodDims[index]) : "";
            if (val.toLowerCase() === "true") {
              exactTrue = val;
              break;
            }
    ...
    root.innerHTML = `<div style="color:red; font:12px Arial;">Hata: ${e.message}</div>`;
  }
}
