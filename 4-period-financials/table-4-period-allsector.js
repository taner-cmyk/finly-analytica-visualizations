...
let userSelectedDate1 = null;
let userSelectedDate2 = null;
let userSelectedDate3 = null;
let userSelectedDate4 = null;
...
function formatOption(rawDate) {
    let str = String(rawDate);
    return str.length === 8 ? `${str.substring(0, 4)}/${str.substring(4, 6)}` : str;
}

function renderTable() {
    const root = getRoot();
    if (!currentData || !currentData.tables || !currentData.tables.DEFAULT) return;

    const tableData = currentData.tables.DEFAULT;
    const fields = currentData.fields;

    // --- SEKTÖR KONTROLÜ VE METRİK SEÇİMİ BAŞLANGICI ---
    let currentSector = "";
    if (tableData.length > 0 && tableData[0].sectorDim) {
        currentSector = String(tableData[0].sectorDim[0]).toLowerCase();
    }

    const isBank = currentSector.includes("bank");
    ...
    if (activeMetrics.length === 0) {
        root.innerHTML = `<div style="padding: 20px; text-align:center; font-family: sans-serif; color: #555;">
            Lütfen Looker Studio panelinden <b>${isBank ? 'Metrics A (Banka)' : 'Metrics B (Banka Dışı)'}</b> alanına metrik ekleyin.
        </div>`;
        return;
    }
    // --- SEKTÖR KONTROLÜ BİTİŞ ---

    // Tarihleri al ve sırala
    let availableDates = [...new Set(tableData.map(row => row.dateDim[0]))];
    availableDates.sort((a, b) => String(b).localeCompare(String(a)));

    // Varsayılan 4 Dönem
    if (!userSelectedDate1 || !availableDates.includes(userSelectedDate1)) userSelectedDate1 = availableDates[0];
    if (!userSelectedDate2 || !availableDates.includes(userSelectedDate2)) userSelectedDate2 = availableDates[1] || availableDates[0];
    if (!userSelectedDate3 || !availableDates.includes(userSelectedDate3)) userSelectedDate3 = availableDates[2] || availableDates[0];
    if (!userSelectedDate4 || !availableDates.includes(userSelectedDate4)) userSelectedDate4 = availableDates[3] || availableDates[0];

    // Seçili tarihlere ait satırları bul
    const rowData1 = tableData.find(row => row.dateDim[0] === userSelectedDate1);
    const rowData2 = tableData.find(row => row.dateDim[0] === userSelectedDate2);
    const rowData3 = tableData.find(row => row.dateDim[0] === userSelectedDate3);
    const rowData4 = tableData.find(row => row.dateDim[0] === userSelectedDate4);

    // CSS ile Scroll Bar genişliği sıfırlandı, kayma engellendi
    let html = `
        <style>
            ...
            /* Firefox ve IE/Edge için scroll bar gizleme */
            body, html, #finly-table-container {
        ...
        <table>
            <thead>
                <tr>
                    <th>Finansal Kalemler<br><span class="sub-title" style="margin-left:0;">Bin TRY</span></th>
                    <th>
                        <div class="dropdown-wrapper">
                            <div class="dropdown-trigger" id="trigger-1">${formatOption(userSelectedDate1)} ▾</div>
                            <div id="dd1" class="dropdown-content">
                                ${availableDates.map(d => `<div class="dropdown-item date-opt-1 ${d === userSelectedDate1 ? 'selected' : ''}" data-value="${d}">${formatOption(d)}</div>`).join('')}
                            </div>
                        </div>
                    </th>
                    <th>
                        <div class="dropdown-wrapper">
                            <div class="dropdown-trigger" id="trigger-2">${formatOption(userSelectedDate2)} ▾</div>
                            <div id="dd2" class="dropdown-content">
                                ${availableDates.map(d => `<div class="dropdown-item date-opt-2 ${d === userSelectedDate2 ? 'selected' : ''}" data-value="${d}">${formatOption(d)}</div>`).join('')}
                            </div>
                        </div>
                    </th>
                    <th>
                        <div class="dropdown-wrapper">
                            <div class="dropdown-trigger" id="trigger-3">${formatOption(userSelectedDate3)} ▾</div>
                            <div id="dd3" class="dropdown-content">
                                ${availableDates.map(d => `<div class="dropdown-item date-opt-3 ${d === userSelectedDate3 ? 'selected' : ''}" data-value="${d}">${formatOption(d)}</div>`).join('')}
                            </div>
                        </div>
                    </th>
                    <th>
                        <div class="dropdown-wrapper">
                            <div class="dropdown-trigger" id="trigger-4">${formatOption(userSelectedDate4)} ▾</div>
                            <div id="dd4" class="dropdown-content">
                                ${availableDates.map(d => `<div class="dropdown-item date-opt-4 ${d === userSelectedDate4 ? 'selected' : ''}" data-value="${d}">${formatOption(d)}</div>`).join('')}
                            </div>
                        </div>
                    </th>
                </tr>
        ...
        const raw1 = rowData1 && rowData1[metricKey] ? rowData1[metricKey][i] : null;
        const raw2 = rowData2 && rowData2[metricKey] ? rowData2[metricKey][i] : null;
        const raw3 = rowData3 && rowData3[metricKey] ? rowData3[metricKey][i] : null;
        const raw4 = rowData4 && rowData4[metricKey] ? rowData4[metricKey][i] : null;

        if (
            (raw1 === null || raw1 === undefined || raw1 === "") &&
            (raw2 === null || raw2 === undefined || raw2 === "") &&
            (raw3 === null || raw3 === undefined || raw3 === "") &&
            (raw4 === null || raw4 === undefined || raw4 === "")
        ) {
            return '';
        }

        const v1 = raw1 !== null && raw1 !== "" ? Number(raw1) : null;
        const v2 = raw2 !== null && raw2 !== "" ? Number(raw2) : null;
        const v3 = raw3 !== null && raw3 !== "" ? Number(raw3) : null;
        const v4 = raw4 !== null && raw4 !== "" ? Number(raw4) : null;

        return `<tr>
                                <td>${m.name}</td>
                                <td class="${v1 < 0 ? 'val-negative' : ''}">${v1 !== null ? v1.toLocaleString('tr-TR') : '-'}</td>
                                <td class="${v2 < 0 ? 'val-negative' : ''}">${v2 !== null ? v2.toLocaleString('tr-TR') : '-'}</td>
                                <td class="${v3 < 0 ? 'val-negative' : ''}">${v3 !== null ? v3.toLocaleString('tr-TR') : '-'}</td>
                                <td class="${v4 < 0 ? 'val-negative' : ''}">${v4 !== null ? v4.toLocaleString('tr-TR') : '-'}</td>
                            </tr>`;
    }).join('')}
            </tbody>
        </table>
    ...
    // EVENT LISTENERS
    document.getElementById('trigger-1').addEventListener('click', (e) => {
        e.stopPropagation();
    ...
    document.getElementById('trigger-2').addEventListener('click', (e) => {
        e.stopPropagation();
        document.getElementById('dd1').classList.remove('show');
        document.getElementById('dd3').classList.remove('show');
        document.getElementById('dd4').classList.remove('show');
        document.getElementById('dd2').classList.toggle('show');
    });

    document.getElementById('trigger-3').addEventListener('click', (e) => {
        e.stopPropagation();
        document.getElementById('dd1').classList.remove('show');
        document.getElementById('dd2').classList.remove('show');
        document.getElementById('dd4').classList.remove('show');
        document.getElementById('dd3').classList.toggle('show');
    });

    document.getElementById('trigger-4').addEventListener('click', (e) => {
        e.stopPropagation();
        document.getElementById('dd1').classList.remove('show');
        document.getElementById('dd2').classList.remove('show');
        document.getElementById('dd3').classList.remove('show');
        document.getElementById('dd4').classList.toggle('show');
    });

    document.querySelectorAll('.date-opt-1').forEach(item => {
        item.addEventListener('click', () => { userSelectedDate1 = item.getAttribute('data-value'); renderTable(); });
    });
    document.querySelectorAll('.date-opt-2').forEach(item => {
        item.addEventListener('click', () => { userSelectedDate2 = item.getAttribute('data-value'); renderTable(); });
    });
    document.querySelectorAll('.date-opt-3').forEach(item => {
        item.addEventListener('click', () => { userSelectedDate3 = item.getAttribute('data-value'); renderTable(); });
    });
    document.querySelectorAll('.date-opt-4').forEach(item => {
        item.addEventListener('click', () => { userSelectedDate4 = item.getAttribute('data-value'); renderTable(); });
    });
}

// 1. İFRAME İÇİ BOŞLUĞA TIKLAYINCA KAPATMA
document.addEventListener('click', (e) => {
    if (!e.target.closest('.dropdown-wrapper')) {
        const d1 = document.getElementById('dd1');
        const d2 = document.getElementById('dd2');
        const d3 = document.getElementById('dd3');
        const d4 = document.getElementById('dd4');
        if (d1) d1.classList.remove('show');
        if (d2) d2.classList.remove('show');
        if (d3) d3.classList.remove('show');
        if (d4) d4.classList.remove('show');
    }
});

// 2. İFRAME DIŞINA TIKLAYINCA KAPATMA
